...
const getAllTrips = async (request, response) => {
    try {
        ...
        const results = await pool.query("SELECT t.* FROM trips_destinations td JOIN trips t ON td.trip_id = t.id WHERE td.destination_id = $1", [destination_id]);
        response.status(200).json(results.rows)
        console.log(results.rows) 
    } catch (error) {
        response.status(409).json({error: error.message})
...
const getAllDestinations  = async (request, response) => {
    try {
        ...
        const results = await pool.query("SELECT d.* FROM trips_destinations td JOIN destinations d ON td.destination_id = d.id WHERE td.trip_id = $1", [trip_id]);
        response.status(200).json(results.rows)
        console.log(results.rows)
    } catch (error) {
        response.status(409).json({error: error.message})
